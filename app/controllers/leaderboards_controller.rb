...
if @leaderboard == "All Leaderboards" || @leaderboard == nil
   if @category == "All Categories"
      @user_list = User.order("points DESC").where(:hasVideo => true).page(params[:page]).per_page(5)
   else 
      @cat = Category.find_by_category_name(@category)
      @user_list = @cat.users.order("points DESC").page(params[:page]).per_page(5)     
   end
else 
      @lead = Leaderboard.find_by_leaderboard_name(@leaderboard)
      @board_id = @lead.id 
      @user_list = @lead.users.order("points DESC").page(params[:page]).per_page(5)  
end 
