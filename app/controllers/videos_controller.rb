...
def upload
    ...
    @cat = Category.find_by_category_name(params[:category])					# finds category 	
     if params[:leaderboard] != "Create New Leaderboard"                        	        # if the user isn't creating a new leaderboard   
        ...
        @video.update_attributes(:leaderboard_id => @lead.id, :category_id => @cat.id)  	# add the video to it and the category              
    else 
